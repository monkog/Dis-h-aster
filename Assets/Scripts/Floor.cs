...
public class Floor : MonoBehaviour
{
	public void AdaptFloorCollider(List<DishInstance> visibleDishes)
	{
		...
		leftCollider.points = leftCollider.points.Take(leftCollider.pointCount - 1).Concat(new[] { new Vector2(minX - 0.5f, leftBound.y) }).ToArray();
		rightCollider.points = (new[] { new Vector2(maxX + 0.5f, rightBound.y) }).Concat(rightCollider.points.Skip(1)).ToArray();
	}
}